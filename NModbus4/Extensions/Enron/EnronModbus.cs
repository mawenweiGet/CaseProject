namespace Modbus.Extensions.Enron
{
    ...
    public static class EnronModbus
    {
        ...
        public static int[] ReadHoldingRegisters32(this ModbusMaster master, byte slaveAddress, ushort startAddress,
        ...
        public static int[] ReadInputRegisters32(this ModbusMaster master, byte slaveAddress, ushort startAddress,
            ushort numberOfPoints)
        {
            if (master == null)
                throw new ArgumentNullException("master");
        ...
        /// </summary>
        private static IEnumerable<short> Convert(int[] registers)
        {
            foreach (var register in registers)
            {
                // low order value
                yield return BitConverter.ToInt16(BitConverter.GetBytes(register), 0);

                // high order value
                yield return BitConverter.ToInt16(BitConverter.GetBytes(register), 2);
            }
        }
        ...
        /// </summary>
        private static IEnumerable<int> Convert(short[] registers)
        {
            for (int i = 0; i < registers.Length; i++)
            {
                yield return ModbusUtility.GetInt32(registers[i + 1], registers[i]);
                i++;
            }
        }
